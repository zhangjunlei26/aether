...
typedef struct {
    ...
    Mailbox mailbox;
    void (*step)(void*);
    int count;  // Changed from atomic_int - no cross-thread access in single-core bench
    long start_time;
...
void bench_actor_step(BenchActor* self) {
    ...
    while (mailbox_receive(&self->mailbox, &msg)) {
        self->count++;  // Plain increment - 5.74x faster than atomic!
    }
...
void bench_contention() {
    ...
    while (atomic_load(&target->count) < TOTAL && (get_time_ms() - start) < 10000) {
        sleep_ms(1);
    }
    
    long end = get_time_ms();
    int processed = atomic_load(&target->count);
    double elapsed = (end - start) / 1000.0;
