...
static void scan_init(scanner_t *sp, const char *src, int len, char *errbuf, int errbufsz);
...
static toml_datum_t *tab_emplace(toml_datum_t *tab, span_t key, const char **reason) {
    ...
    for (int i = 0; i < N; i++) {
        if (tab->u.tab.len[i] == key.len && 0 == memcmp(tab->u.tab.key[i], key.ptr, key.len)) {
    ...
    {
        toml_datum_t *value = REALLOC(tab->u.tab.value, sizeof(*value) * align8(N + 1));
        if (!value) {
            *reason = "out of memory";
            return NULL;
        }
...
static int tab_add(toml_datum_t *tab, span_t newkey, toml_datum_t newvalue, const char **reason) {
    assert(tab->type == TOML_TABLE);
...
static toml_datum_t mkdatum(toml_type_t ty) {
    toml_datum_t ret = {0};
    ret.type = ty;
    if (ty == TOML_DATE || ty == TOML_TIME || ty == TOML_DATETIME || ty == TOML_DATETIMETZ) {
        ret.u.ts.year = -1;
        ret.u.ts.month = -1;
        ret.u.ts.day = -1;
        ret.u.ts.hour = -1;
...
static int datum_copy(toml_datum_t *dst, toml_datum_t src, pool_t *pool, const char **reason) {
...
static int datum_merge(toml_datum_t *dst, toml_datum_t src, pool_t *pool, const char **reason) {
...
static bool datum_equiv(toml_datum_t a, toml_datum_t b) {
    ...
    int N;
    switch (a.type) {
        case TOML_STRING: return a.u.str.len == b.u.str.len && 0 == memcmp(a.u.str.ptr, b.u.str.ptr, a.u.str.len);
        case TOML_INT64: return a.u.int64 == b.u.int64;
        case TOML_FP64: return a.u.fp64 == b.u.fp64 || (isnan(a.u.fp64) && isnan(b.u.fp64));
        case TOML_BOOLEAN: return !!a.u.boolean == !!b.u.boolean;
        case TOML_DATE: return a.u.ts.year == b.u.ts.year && a.u.ts.month == b.u.ts.month && a.u.ts.day == b.u.ts.day;
        case TOML_TIME: return a.u.ts.hour == b.u.ts.hour && a.u.ts.minute == b.u.ts.minute && a.u.ts.second == b.u.ts.second && a.u.ts.usec == b.u.ts.usec;
        case TOML_DATETIME:
            return a.u.ts.year == b.u.ts.year && a.u.ts.month == b.u.ts.month && a.u.ts.day == b.u.ts.day && a.u.ts.hour == b.u.ts.hour && a.u.ts.minute == b.u.ts.minute &&
                   a.u.ts.second == b.u.ts.second && a.u.ts.usec == b.u.ts.usec;
        case TOML_DATETIMETZ:
            return a.u.ts.year == b.u.ts.year && a.u.ts.month == b.u.ts.month && a.u.ts.day == b.u.ts.day && a.u.ts.hour == b.u.ts.hour && a.u.ts.minute == b.u.ts.minute &&
                   a.u.ts.second == b.u.ts.second && a.u.ts.usec == b.u.ts.usec && a.u.ts.tz == b.u.ts.tz;
        case TOML_ARRAY:
            N = a.u.arr.size;
            if (N != b.u.arr.size) {
                return false;
...
toml_result_t toml_parse_file(FILE *fp) {
    ...
    while (!feof(fp)) {
        ...
        if (ferror(fp)) {
            snprintf(result.errmsg, sizeof(result.errmsg), "%s", errno ? strerror(errno) : "Error reading file");
            FREE(buf);
            return result;
        }
    }
...
toml_result_t toml_parse(const char *src, int len) {
    ...
    if (src[len]) {
        snprintf(result.errmsg, sizeof(result.errmsg), "src[] must be NUL terminated");
        goto bail;
    }
    ...
    if (toml_option.check_utf8) {
        ...
        for (int i = 0; i < len;) {
            ...
            if (n < 0) {
                snprintf(result.errmsg, sizeof(result.errmsg), "invalid UTF8 char on line %d", line);
                goto bail;
            }
            if (0xD800 <= ch && ch <= 0xDFFF) {
                // explicitly prohibit surrogates (non-scalar unicode code point)
                snprintf(result.errmsg, sizeof(result.errmsg), "invalid UTF8 char \\u%04x on line %d", ch, line);
                goto bail;
            }
            line += (ch == '\n' ? 1 : 0);
            i += n;
    ...
    // Alloc memory pool
    pp->pool = pool_create(len + 10); // add some extra bytes for NUL term and safety
    if (!pp->pool) {
        snprintf(result.errmsg, sizeof(result.errmsg), "out of memory");
        goto bail;
    }
...
static int token_to_timestamp(parser_t *pp, token_t tok, toml_datum_t *ret) {
    (void)pp;
    static const toml_type_t map[] = {[TOK_TIME] = TOML_TIME, [TOK_DATE] = TOML_DATE, [TOK_DATETIME] = TOML_DATETIME, [TOK_DATETIMETZ] = TOML_DATETIMETZ};
...
static int parse_key(parser_t *pp, token_t tok, keypart_t *ret_keypart) {
    ...
    if (parse_norm(pp, tok, &kpspan[n])) {
        return SETERROR(pp->ebuf, tok.lineno, "unable to normalize string; probably a unicode issue");
    }
    n++;
...
static toml_datum_t *descend_keypart(parser_t *pp, int lineno, toml_datum_t *toptab, keypart_t *keypart, bool stdtabexpr) {
    ...
    for (int i = 0; i < keypart->nspan; i++) {
        ...
        if (value->type == TOML_ARRAY) {
            ...
            if (value->u.arr.size <= 0) {
                SETERROR(pp->ebuf, lineno, "array %s has no elements", keypart->span[i].ptr);
                return NULL;
            }
            ...
            // It must be a table!
            if (value->type != TOML_TABLE) {
                SETERROR(pp->ebuf, lineno, "array %s must be array of tables", keypart->span[i].ptr);
                return NULL;
            }
            tab = value; // descend
            continue;
        ...
        // key not found
        SETERROR(pp->ebuf, lineno, "cannot locate table at key %s", keypart->span[i].ptr);
        return NULL;
    }
    ...
    *ret_datum = mkdatum(TOML_ARRAY);
    ...
    for (;;) {
        ...
        if (tok.toktyp == TOK_COMMA) {
            ...
            return SETERROR(pp->ebuf, tok.lineno, "syntax error while parsing array: unexpected comma");
        }

        // Not a comma, but need a comma: error!
        if (need_comma) {
            return SETERROR(pp->ebuf, tok.lineno, "syntax error while parsing array: missing comma");
        }
...
static int parse_inline_table(parser_t *pp, token_t tok, toml_datum_t *ret_datum) {
    ...
    *ret_datum = mkdatum(TOML_TABLE);
    ...
    for (;;) {
        ...
        toml_datum_t *tab = descend_keypart(pp, keylineno, ret_datum, &keypart, false);
        if (!tab) {
            return -1;
        }
...
static int parse_val(parser_t *pp, token_t tok, toml_datum_t *ret) {
    ...
    switch (tok.toktyp) {
        ...
        case TOK_MLSTRING:
        case TOK_LITSTRING:
        case TOK_MLLITSTRING: return token_to_string(pp, tok, ret);
        case TOK_TIME:
        case TOK_DATE:
        case TOK_DATETIME:
        case TOK_DATETIMETZ: return token_to_timestamp(pp, tok, ret);
        case TOK_INTEGER: return token_to_int64(pp, tok, ret);
        case TOK_FLOAT: return token_to_fp64(pp, tok, ret);
        case TOK_BOOL: return token_to_boolean(pp, tok, ret);
...
static int parse_std_table_expr(parser_t *pp, token_t tok) {
    ...
    toml_datum_t *tab = descend_keypart(pp, keylineno, &pp->toptab, &keypart, true);
    if (!tab) {
        return -1;
    }
    ...
    for (int i = 0; i < keypart.nspan; i++) {
        ...
        if (value->type == TOML_ARRAY) {
            if (value->flag & FLAG_INLINED) {
                return SETERROR(pp->ebuf, keylineno, "cannot expand array %s", curkey.ptr);
            }
            if (value->u.arr.size <= 0) {
                return SETERROR(pp->ebuf, keylineno, "array %s has no elements", curkey.ptr);
            }
            value = &value->u.arr.elem[value->u.arr.size - 1];
            if (value->type != TOML_TABLE) {
                return SETERROR(pp->ebuf, keylineno, "array %s must be array of tables", curkey.ptr);
            }
            tab = value;
            continue;
        }
        ...
        // keypart not found
        return SETERROR(pp->ebuf, keylineno, "cannot locate table at key %s", curkey.ptr);
    }
    ...
    for (int i = 0; i < keypart.nspan - 1; i++) {
        ...
        if (j < 0) {
            if (i > 0 && (tab->flag & FLAG_EXPLICIT)) {
                return SETERROR(pp->ebuf, keylineno, "cannot extend a previously defined table using dotted expression");
            }
            toml_datum_t newtab = mkdatum(TOML_TABLE);
            if (tab_add(tab, keypart.span[i], newtab, &reason)) {
                return SETERROR(pp->ebuf, keylineno, "%s", reason);
        ...
        if (value->type == TOML_ARRAY) {
            return SETERROR(pp->ebuf, keylineno, "encountered previously declared array '%s'", keypart.span[i].ptr);
        }
        return SETERROR(pp->ebuf, keylineno, "cannot locate table at '%s'", keypart.span[i].ptr);
    }
...
static int parse_norm(parser_t *pp, token_t tok, span_t *ret_span) {
    ...
    switch (tok.toktyp) {
        ...
        default: return SETERROR(pp->ebuf, 0, "internal: arg must be a string");
    }
    ...
    while (*p) {
        ...
        switch (p[1]) {
            ...
            case 'x': {
                ...
                if (n < 0) {
                    return SETERROR(pp->ebuf, tok.lineno, "error converting UCS %s to UTF8", buf);
                }
                dst += n;
                p += 2 + 2; // \xNN
                continue;
            ...
            case '\r':
                ...
                if (*p != '\n') {
                    return SETERROR(pp->ebuf, tok.lineno, "unexpected char after line-ending backslash");
                }
                // fallthru
            case '\n':
    ...
    while (1) {
        if (S_MATCH3('"')) {
            if (S_MATCH4('"')) {
                ...
                if (S_MATCH6('"')) {
                    return SETERROR(sp->ebuf, sp->lineno, "detected sequences of 3 or more double quotes");
                } else {
                    ; // no problem
                }
            } else {
        ...
        if (top) {
            for (int i = 0; i < top; i++) {
                if (!is_hex_char(S_GET())) {
                    return SETERROR(sp->ebuf, sp->lineno, "expect %d hex digits after \\%c", top, ch);
                }
            }
            continue;
        }
    ...
    *tok = mktoken(sp, TOK_STRING);
    ...
    while (!S_MATCH('"')) {
        ...
        switch (ch) {
            ...
            case 'u': top = 4; break;
            case 'U': top = 8; break;
            default: return SETERROR(sp->ebuf, sp->lineno, "bad escape char in string");
        }
        for (int i = 0; i < top; i++) {
            if (!is_hex_char(S_GET())) {
                return SETERROR(sp->ebuf, sp->lineno, "expect %d hex digits after \\%c", top, ch);
    ...
    while (1) {
        if (S_MATCH3('\'')) {
            if (S_MATCH4('\'')) {
                ...
                if (S_MATCH6('\'')) {
                    return SETERROR(sp->ebuf, sp->lineno, "sequences of 3 or more single quotes");
                } else {
                    ; // no problem
                }
            } else {
        ...
        int ch = S_GET();
        if (ch == TOK_FIN) {
            return SETERROR(sp->ebuf, sp->lineno, "unterminated multiline lit string");
        }
        if (!(is_valid_char(ch) || (ch && strchr(" \t\n", ch)))) {
            return SETERROR(sp->ebuf, sp->lineno, "invalid char in string");
        }
...
static int read_time(const char *p, int *hour, int *minute, int *second, int *usec) {
    const char *pp = p;
    int n;
...
static int scan_timestamp(scanner_t *sp, token_t *tok) {
    ...
    if (!((p[0] == 'T' || p[0] == ' ' || p[0] == 't') && isdigit(p[1]) && isdigit(p[2]) && p[3] == ':')) {
        goto done; // date only
    }
    ...
    *tok = mktoken(sp, toktyp);
    ...
    switch (tok->toktyp) {
        ...
        case TOK_DATETIMETZ:
            ...
            break;
        default: assert(0); return SETERROR(sp->ebuf, lineno, "internal error");
    }
...
static inline bool test_time(const char *p, const char *endp) { return &p[2] < endp && isdigit(p[0]) && isdigit(p[1]) && p[2] == ':'; }

// Check if the next token may be DATE
static inline bool test_date(const char *p, const char *endp) { return &p[4] < endp && isdigit(p[0]) && isdigit(p[1]) && isdigit(p[2]) && isdigit(p[3]) && p[4] == '-'; }

// Check if the next token may be BOOL
static inline bool test_bool(const char *p, const char *endp) { return &p[0] < endp && (*p == 't' || *p == 'f'); }
...
again:
    *tok = mktoken(sp, TOK_FIN);
    ...
    tok->str.len = 1;
    switch (ch) {
        case '\n': tok->toktyp = TOK_ENDL; break;

        case ' ':
        case '\t': goto again; // skip whitespace
        ...
        case '#':
            ...
            while (!S_MATCH('\n')) {
                ...
                if (ch == TOK_FIN)
                    break;
                if ((0 <= ch && ch <= 0x8) || (0x0a <= ch && ch <= 0x1f) || (ch == 0x7f)) {
                    return SETERROR(sp->ebuf, sp->lineno, "bad control char in comment");
                }
            }
            goto again; // skip comment

        case '.': tok->toktyp = TOK_DOT; break;

        case '=': tok->toktyp = TOK_EQUAL; break;

        case ',': tok->toktyp = TOK_COMMA; break;
        ...
        case '{': tok->toktyp = TOK_LBRACE; break;

        case '}': tok->toktyp = TOK_RBRACE; break;
...
static int check_overflow(scanner_t *sp, token_t *tok) {
    switch (tok->toktyp) {
        case TOK_LBRACK:
            ...
            break;
        case TOK_RBRACK: sp->bracket_level--; break;
        case TOK_LBRACE:
            sp->brace_level++;
            ...
            break;
        case TOK_RBRACE: sp->brace_level--; break;
        default: break;
    }
    return 0;
}
